...
contract ComputationEquivalenceTest is Test {
    ...
    function testComputationalEquivalence() public {
        ...
        for (uint scenario = 1; scenario <= 5; scenario++) {
            ...
            // --- Verify equivalence ---
            assertEq(
                traditionalVotingPower, 
                optimizedVotingPower, 
                "Voting power should be identical between methods"
            );
            
            assertEq(
                traditionalVotePassed, 
                optimizedVotePassed, 
                "Vote outcome should be identical between methods"
            );
